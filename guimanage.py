...
function saveConfig(){collectConfig();var btn=document.getElementById("saveBtn");btn.disabled=true;btn.classList.add("saving");addLog("保存配置中...","log-info");apiPost("/api/config",cfg).then(function(res){btn.disabled=false;btn.classList.remove("saving");if(res&&res.ok){toast("配置已保存","ok");addLog("配置保存成功","log-ok");return fetchConfig()}else{var msg=res&&res.error?res.error:"未知错误";toast("保存失败: "+msg,"fail");addLog("保存失败: "+msg,"log-err")}})}
